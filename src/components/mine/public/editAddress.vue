<template>
	<div class="edit-address">
		...
		<div class="consignee input-box gap">
			<span>收货人</span>
			<input type="text" placeholder="收货人" v-model="editAddressForm.consignee">
		</div>
		<div class="phone input-box gap">
			<span>手机号码</span>
			<input type="text" placeholder="手机号码" v-model="editAddressForm.phone">
		</div>
		<div class="area gap">
			<span>所在地区</span>
			<el-cascader
		...
		<div class="address input-box gap">
			<span>详细地址</span>
			<input type="text" placeholder="详细地址:小区、门牌号" v-model="editAddressForm.address">
		</div>
		<div class="sign gap">
			<span>地址标签</span>
		...
		<div class="is_default gap">
			<span>设为默认地址</span>
			<el-switch 
			v-model="editAddressForm.is_default"
			active-color="#13ce66"
			inactive-color="#eee">
			>
			</el-switch>
		...
		<div class="save gap">
			<el-button type="warning" @click="editAddress">保存</el-button>
		</div>
	...
	export default {
		data(){
			return {
				...
				area:[110,110100000000,110101000000,110101001000,110101001001],
				provinceList:[],
				sign:['家','公司','学校'],
				chooseSignText:"",
				newsign:"",
				...
				areaProp:{
					...
					lazyLoad:(node,resolve)=>{
						...
						const { data } = node;
						if (level == 1){
							this.editAddressForm.area = '';
							this.editAddressForm.area += node.data.province_name;
							this.$homehttp({
		...
		methods:{
			...
			areaChange(e){
				...
				this.editAddressForm.city = e[1];
				this.editAddressForm.county = e[2];
				this.editAddressForm.town = e[3];
				this.editAddressForm.village = e[4];
				this.$homehttp({
					url:'village?village_id='+e[4]
				}).then(result=>{
					const {code,msg,data} = result.data;
					if (code == 200) {
						this.editAddressForm.area += data[0].village_name;
					}
				})
			},
			chooseSign(index){
				...
				this.$homehttp({
					...
					if (code == 200) {
						...
						if (this.editAddressForm.is_default === 0) {
							this.editAddressForm.is_default = false;
						}else{
							this.editAddressForm.is_default = true;
						}
					}else{
			...
			editAddress(){
				this.editAddressForm.sign = this.chooseSignText;
				if (this.editAddressForm.is_default === true) {
				...
				this.$homehttp({
					url:'address/'+this.id,
					method:'put',
					data:this.editAddressForm
				}).then(result=>{
					const {code,msg} = result.data;
					if (code == 200) {
						this.$message({message:'编辑收货地址成功',type:'success'});
					}else{
